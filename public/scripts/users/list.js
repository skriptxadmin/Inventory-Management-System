jQuery(function () {
  ...
  const roles$ = body$.find("select[name=roles]");
  ...
  const dtInstance = table$.DataTable({
    ...
    ajax: function (data, callback, settings) {
      data.role_id = roles$.val();
      jQuery.ajax({
        endpoint: "users/list", // Your CI4 controller endpoint
        method: "POST",
  ...
  table$.on("click", ".btn-delete", function () {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to remove ${row.fullname} user? You won't be able to revert this!`,
      icon: "warning",
