jQuery(function () {
  ...
  const dtInstance = table$.DataTable({
    ...
    ajax: function (data, callback, settings) {
      jQuery.ajax({
        endpoint: "unit-of-measures/list", // Your CI4 controller endpoint
        method: "POST",
  ...
  table$.on("click", ".btn-delete", function () {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to remove ${row.name} unit? You won't be able to revert this!`,
      icon: "warning",
  ...
  function removeRole(role) {
    jQuery.ajax({
      endpoint: "unit-of-measures/" + role.id,
      method: "DELETE",
