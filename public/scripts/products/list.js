jQuery(function () {
  ...
  const dtInstance = table$.DataTable({
    ...
    ajax: function (data, callback, settings) {
      jQuery.ajax({
        endpoint: "products/list", // Your CI4 controller endpoint
        method: "POST",
  ...
  table$.on("click", ".btn-delete", function () {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to remove ${row.name} product? You won't be able to revert this!`,
      icon: "warning",
  ...
  function removeProduct(role) {
    jQuery.ajax({
      endpoint: "products/" + role.id,
      method: "DELETE",
