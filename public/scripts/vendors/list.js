jQuery(function () {
  const body$ = jQuery("body.vendors-list");

  if (_.isEmpty(body$)) {
    console.error("body class vendors-list not found");

    return;
  }

  const table$ = body$.find("table.vendors-dt");
  ...
  const dtInstance = table$.DataTable({
    ...
    ajax: function (data, callback, settings) {
      jQuery.ajax({
        endpoint: "vendors/list", // Your CI4 controller endpoint
        method: "POST",
  ...
  table$.on("click", ".btn-delete", function () {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to remove ${row.name} vendor? You won't be able to revert this!`,
      icon: "warning",
